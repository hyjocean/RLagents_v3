...
from MAPF_env.agents.kinematics import Agent
...
class World(object):
    ...
    def step(self, dt: float) -> None:
        ...
        for i, agent in self.agents.items():
            for other in [self.agents.values()][i + 1:]:
                agent.handle_collisions(other, dt)
    ...
    def reset_world(self):
        ...
        self.state, self.goals_map = self.map_generator.generator()
        # detect manual world
        if (self.state > 0).any():
            ...
            self.agents = {i: copy.deepcopy(Agent(self.state, None))
        ...
        self.corridor_map = {}  # TODO: corrider_map是设置什么的？
    ...
    def put_goals(self, id_list, manual_pos=None):
        ...
        """
        # TODO: 需要注意下为什么会覆盖previous_goals，重新生成新目标？
...
class MAPFEnv(gym.Env):
    ...
    def __init__(self, observer, map_generator, agents_count=None,
        ...
        self.isStandingOnGoal = {
            i: False for i in range(1, self.agents_count + 1)}

        self.individual_rewards = {
            i: 0 for i in range(1, self.agents_count + 1)}
        # self.mutex = Lock()
        self.GIF_frame = []
        if IsDiagonal:
            self.action_space = spaces.Tuple(
                [spaces.Discrete(self.agents_count), spaces.Discrete(9)])
        else:
            self.action_space = spaces.Tuple(
                [spaces.Discrete(self.agents_count), spaces.Discrete(5)])
    ...
    def getGoals(self):
        return {i: self.world.agents[i].goal_pos for i in range(1, self.agents_count + 1)}

    def getStatus(self):
        return {i: self.world.agents[i].status for i in range(1, self.agents_count + 1)}

    def getPositions(self):
        return {i: self.world.agents[i].position for i in range(1, self.agents_count + 1)}

    def getLastMovements(self):
        return {i: self.world.agents[i].position_history(-1) for i in range(1, self.agents_count + 1)}

    def set_world(self):

        self.world = World(
            self.map_generator, agents_count=self.agents_count, isDiagonal=self.IsDiagonal)
    ...
    def _observe(self, handles=None):
        ...
        if handles is None:
            self.obs_dict = self.observer.get_many(
                list(range(1, self.agents_count + 1)))
        elif handles in list(range(1, self.agents_count + 1)):
            self.obs_dict = self.observer.get_many([handles])
        elif set(handles) == set(handles) & set(list(range(1, self.agents_count + 1))):
            self.obs_dict = self.observer.get_many(handles)
        else:
    ...
    def step_all(self, movement_dict):
        ...
        free_agents = list(range(1, self.agents_count + 1))
    ...
    def expert_until_first_goal(self, inflation=2.0, time_limit=60.0):
        ...
        for i in range(1, self.world.agents_count + 1):
        ...
        #         world, start_positions, goals, inflation, time_limit / 5.0)
    ...
    #         def initColors(agents_count):
    #             c = {
    #                 a + 1: hsv_to_rgb(np.array([a / float(agents_count), 1, 1])) for a in range(agents_count)}
    #             return c
...
#         env = Primal2Env(agents_count=n_agents,
