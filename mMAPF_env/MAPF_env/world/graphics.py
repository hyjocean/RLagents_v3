...
# from MAPF_env.world.lane import LineType, AbstractLane
# from MAPF_env.world.road import Road

import sys
sys.path.append('/home/bld/HK_RL/RLagents')
from ...MAPF_env.utils import Vector
from ..agents.graphics import AgentGraphics
from ..agents.objects import Obstacle, Landmark
...
class WorldSurface(pygame.Surface):
    ...
    WHITE = (255, 255, 255)
    # GREY = (100, 100, 100)
    GREY = (128, 128, 128)#或者 (169, 169, 169)
    GREEN = (50, 200, 0)
    # YELLOW = (200, 200, 0)
    PINK = (255, 182, 193)
    BLUE = (173, 216, 230)
...
class WorldGraphics(object):
    ...
    @classmethod
    def display(cls, world, surface) -> None:
        CELL_SIZE = int(surface.scaling)
        GRID_SIZE = world.state.shape[0]
        GRID_WIDTH, GRID_HEIGHT = GRID_SIZE*CELL_SIZE,  GRID_SIZE*CELL_SIZE

        obstacle_map = np.where((world.state == 0) | (world.state == -1), world.state, 0)
        start_map = np.where((world.state != 0) & (world.state != -1), world.state, 0)
        target_map = world.goals
    
        grid = np.array([[surface.WHITE] * GRID_SIZE for _ in range(GRID_SIZE)])
        
        grid[obstacle_map==-1] = surface.BLACK
        grid[start_map > 0] = surface.YELLOW
        grid[target_map > 0] = surface.BLUE
        ...
        pygame.draw.line(surface, surface.BLACK, (GRID_WIDTH-1, 0), (GRID_WIDTH-1, GRID_HEIGHT), cls.LINE_WIDTH)
        pygame.draw.line(surface, surface.BLACK, (0, GRID_HEIGHT-1), (GRID_WIDTH, GRID_HEIGHT-1), cls.LINE_WIDTH)


        # pygame.draw.rect(surface, cls.PINK, (specify_start_grid[1] * cls.CELL_SIZE, specify_start_grid[0] * cls.CELL_SIZE, cls.CELL_SIZE, cls.CELL_SIZE))

        # pygame.draw.line(surface, cls.RED, (specify_end_grid[1]*cls.CELL_SIZE, specify_end_grid[0]*cls.CELL_SIZE), ((specify_end_grid[1]+1)*cls.CELL_SIZE, (specify_end_grid[0]+1)*cls.CELL_SIZE), 1)
        # pygame.draw.line(surface, cls.RED, (specify_end_grid[1]*cls.CELL_SIZE, (specify_end_grid[0]+1)*cls.CELL_SIZE), ((specify_end_grid[1]+1)*cls.CELL_SIZE, specify_end_grid[0]*cls.CELL_SIZE), 1)
        # for x in range(0, cls.GRID_WIDTH, cls.ELL_SIZE):
        #     pygame.draw.line(surface, cls.BLACK, (x, 0), (x, cls.GRID_HEIGHT), cls.LINE_WIDTH)
        # for y in range(0, cls.GRID_HEIGHT, cls.CELL_SIZE):
        #     pygame.draw.line(surface, cls.BLACK, (0, y), (cls.GRID_WIDTH, y), cls.LINE_WIDTH)
...
#         ends = longitudinal + \
#             np.arange(stripes_count) * cls.STRIPE_SPACING + cls.STRIPE_LENGTH
...
#         starts = longitudinal + np.arange(stripes_count) * cls.STRIPE_SPACING
#         ends = longitudinal + \
#             np.arange(stripes_count) * cls.STRIPE_SPACING + cls.STRIPE_SPACING
#         lats = [(side - 0.5) * lane.width_at(s) for s in starts]
#         cls.draw_stripes(lane, surface, starts, ends, lats)
...
#         ends = [longitudinal + stripes_count *
#                 cls.STRIPE_SPACING + cls.STRIPE_LENGTH]
#         lats = [(side - 0.5) * lane.width_at(s) for s in starts]
#         cls.draw_stripes(lane, surface, starts, ends, lats)
...
#                                  (surface.vec2pix(
#                                      lane.position(starts[k], lats[k]))),
#                                  (surface.vec2pix(
#                                      lane.position(ends[k], lats[k]))),
...
#         dots = []
#         for side in range(2):
#             longis = np.clip(s0 + np.arange(stripes_count) *
#                              cls.STRIPE_SPACING, 0, lane.length)
#             lats = [2 * (side - 0.5) * width for _ in longis]
#             new_dots = [surface.vec2pix(lane.position(longi, lat))
...
class MazeGraphics(object):
    ...
    """A visualization of a world."""


    @staticmethod
    def display(world: World, surface: WorldSurface) -> None:
        """
        ...
        world_size = world.map_generator.world_size
...
#         """
#         o = object_
#         s = pygame.Surface((surface.pix(o.LENGTH), surface.pix(
#             o.LENGTH)), pygame.SRCALPHA)  # per-pixel alpha
#         rect = (0, surface.pix(o.LENGTH / 2 - o.WIDTH / 2),
#                 surface.pix(o.LENGTH), surface.pix(o.WIDTH))
