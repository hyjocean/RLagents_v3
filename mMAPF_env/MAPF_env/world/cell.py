...
class AbstractCell(object):  # CHANGE: AbstractLane -> AbstractCell
    ...
    @abstractmethod  # ANNOT: 表面此只能用于继承，不可实例化，子类需重写，不可复用
...
class StraightCell(AbstractCell):  # CHANGE: Lane -> Cell
...
class PolyLane(PolyLaneFixedWidth):
    ...
    def to_config(self) -> dict:
        ...
        ordered_boundary_points = _to_serializable(
            [_to_serializable(p.position)
             for p in reversed(self.left_boundary.poses)]
        )
        ordered_boundary_points += _to_serializable(
