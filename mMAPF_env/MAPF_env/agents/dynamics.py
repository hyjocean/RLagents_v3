...
# class BicycleAgent(Agent):
#     """
#     A dynamical bicycle model, with tire friction and slipping.

#     See Chapter 2 of Lateral Agent Dynamics. Agent Dynamics and Control. Rajamani, R. (2011)
#     """

#     MASS: float = 1  # [kg]
#     LENGTH_A: float = Agent.LENGTH / 2  # [m]
#     LENGTH_B: float = Agent.LENGTH / 2  # [m]
#     INERTIA_Z: float = (
#         1 / 12 * MASS * (Agent.LENGTH**2 + Agent.WIDTH**2)
#     )  # [kg.m2]
...
#     def __init__(
#         self, world: World, position: Vector, heading: float = 0, speed: float = 0
#     ) -> None:
...
#         delta_r = 0
#         theta_vf = np.arctan2(
#             lateral_speed + self.LENGTH_A * yaw_rate, speed)  # (2.27)
#         theta_vr = np.arctan2(
#             lateral_speed - self.LENGTH_B * yaw_rate, speed)  # (2.28)
...
#                 -self.MASS * lateral_speed + self.INERTIA_Z / self.LENGTH_A * yaw_rate
#             )
#         d_lateral_speed = 1 / self.MASS * \
#             (f_yf + f_yr) - yaw_rate * speed  # (2.21)
#         d_yaw_rate = (
...
#         A0 = np.concatenate(
#             (A_top, np.concatenate((np.zeros((2, 2)), A_lat), axis=1)))
#         phi = np.array(
#             [
#                 np.concatenate(
#                     (
...
#     vehicle = BicycleAgent(road=None, position=[0, 5], speed=8.3)
#     xx, uu = [], []
#     from mapf_env.interval import LPV
...
#     dir_x, dir_y = np.cos(xx[:, 2, 0] + uu[:, 0, 0]
#                           ), np.sin(xx[:, 2, 0] + uu[:, 0, 0])
