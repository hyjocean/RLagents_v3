...
if TYPE_CHECKING:
    # from MAPF_env.World.lane import AbstractLane
    from MAPF_env.world import world
...
class WorldObject(ABC):
    ...
    def __init__(self, world: 'world', position: Sequence[float], heading: float = 0, speed: float = 0):
        """
        ...
        #     self.position, self.heading) if self.road else np.nan
    ...
    # @classmethod
    # def make_on_lane(cls, road: 'world', lane_index: LaneIndex, longitudinal: float, speed: Optional[float] = None) \
    #         -> 'WorldObject':
    #     """
    ...
    #     :return: a WorldObject at the specified position
    #     """
    ...
    def handle_collisions(self, other: 'WorldObject', dt: float = 0) -> None:
        """
        ...
        if not (self.collidable and other.collidable):
            return
        intersecting, will_intersect, transition = self._is_colliding(
            other, dt)
        if will_intersect:
    ...
    # def lane_distance_to(self, other: 'WorldObject', lane: 'AbstractLane' = None) -> float:
    #     """
    ...
    def front_distance_to(self, other: "WorldObject") -> float:
