...
class Agent(ABC):
    ...
    def create_random(
        ...
        """
        if speed is None:
            speed = np.random.RandomState().uniform(
                    Agent.DEFAULT_INITIAL_SPEEDS[0], Agent.DEFAULT_INITIAL_SPEEDS[1]
                )
        ...
        v = cls(world, position, goal, heading, speed)
        return v
    ...
    @classmethod
    def create_from(cls, vehicle: "Agent") -> "Agent":
        """
    ...
    def to_dict(
        self, origin_vehicle: "Agent" = None, observe_intentions: bool = True
    ) -> dict:
        d = {
            "presence": 1,
            "x": self.position[0],
    ...
    def __str__(self):
        return "{} #{}: id_{}, {}".format(
            self.__class__.__name__, id(self) % 1000, self.id_label, self.position
        )
