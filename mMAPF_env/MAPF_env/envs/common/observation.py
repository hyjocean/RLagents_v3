...
from ....MAPF_env.utils import distance_to_circle, Vector
# from MAPF_env.agents.controller import MDPAgent
...
class ObservationType(object):
    ...
    @property
    def observer_agent(self):
        """
        The vehicle observing the scene.

        If not set, the first controlled vehicle is used by default.
        """
        return self.__observer_agent or self.env.agent

    @observer_agent.setter
    def observer_agent(self, agent):
        self.__observer_agent = agent
...
class KinematicObservation(ObservationType):
    ...
    def normalize_obs(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self.features_range:
            ...
            self.features_range = {
                "x": [-5.0 * Agent.MAX_SPEED, 5.0 * Agent.MAX_SPEED],
                "y": [-AbstractLane.DEFAULT_WIDTH * len(side_lanes), AbstractLane.DEFAULT_WIDTH * len(side_lanes)],
                "vx": [-2*Agent.MAX_SPEED, 2*Agent.MAX_SPEED],
                "vy": [-2*Agent.MAX_SPEED, 2*Agent.MAX_SPEED]
            }
...
class SingleAgentObservation(ObservationType):

    """Observe an occupancy grid of nearby agents."""

    FEATURES: List[str] = ['poss_map', 'goal_map', 'goals_map', 'obs_map', 'vx', 'vy', 'mag']# v
    GRID_SIZE: List[List[float]] = [[-2.5*5, 2.5*5], [-2.5*5, 2.5*5]]
    GRID_STEP: List[float] = [0.1, 0.1]
    ...
    def __init__(self,
        ...
        """
        super().__init__(env)
        # self.WORLD_SIZE = self.env.world.state.shape[0]
        ...
        # grid_shape = np.asarray(np.floor((self.grid_size[:, 1] - self.grid_size[:, 0]) / self.grid_step),
                                # dtype=np.uint8)
        agent_obs_size =  self.env.observation_size
        grid_shape = (agent_obs_size, agent_obs_size)
        self.grid = np.zeros((len(self.features[:4]), *grid_shape))
        self.goal_vector = np.zeros((1, len(self.features[-3:])))
        
    def space(self) -> spaces.Space:
        # return spaces.Dict(
        #         {'self_pos_map': spaces.Box(0, 1, shape=obs_shape,dtype=np.int8),
        #          'goal_map': spaces.Box(0, 1, shape=obs_shape,dtype=np.int8),
        #          'goals_map': spaces.Box(1, num_agents+1, shape=obs_shape,dtype=np.int8),
        #          'obstacle': spaces.Box(0, 1, shape=obs_shape,dtype=np.int8),
        #          'goal_vector': spaces.Box(-np.inf, np.inf, shape=(1,3), dtype=np.float32),
                 
        #          'speed_map': spaces.Box(0, self.MAX_SPEED, shape=obs_shape,dtype=np.float64),
        #          'direction_map': spaces.Box(-180, 180, shape=obs_shape,dtype=np.float64),
        #                   })
        return spaces.Dict(OrderedDict(
                maps=spaces.Box(-np.inf, np.inf,
                                shape=self.grid.shape, dtype=np.float64),
                goal_vector=spaces.Box(-np.inf, np.inf,
    ...
    def observe(self) -> np.ndarray:
        if not self.env.world:
            return (np.zeros(self.space()['maps'].shape), np.zeros(self.space()['goal_vector'].shape))

    
        # Initialize empty data
        self.grid.fill(np.nan)
        self.goal_vector.fill(np.nan)
        # Get nearby traffic data
        df = pd.DataFrame.from_records(
            [v.to_dict() for v in self.env.world.agents])
        # Normalize
        df = self.normalize(df)
        obs_maps = self.grid
        obs_goalv = self.goal_vector
        if self.clip:
            obs_maps = np.clip(obs_maps, -1, 1)
        obs_maps = np.nan_to_num(obs_maps).astype(self.space()['maps'].dtype)
        obs_goalv =  np.nan_to_num(obs_goalv).astype(self.space()['goal_vector'].dtype)
        obs = OrderedDict(
                {"maps":obs_maps,
                "goal_vector":obs_goalv,
            })
        return obs

class EnvObservation(ObservationType):
    ...
    def __init__(self,
        ...
        # grid_shape = (agent_obs_size, agent_obs_size)
        # self.grid = np.zeros((len(self.features[:4]), *grid_shape))
        # self.goal_vector = np.zeros((len(self.features[-3:])))

        self.env = env
        self.num_agents = env.num_agents

    def space(self) -> Optional[List[spaces.Space]]:
        return MultiAgentObservationSpace([SingleAgentObservation(self.env
                                                                  ).space() for _ in range(self.num_agents)])
 
    def observe(self):
        return [SingleAgentObservation(self.env
                                       ).observe() for _ in range(self.num_agents)]
...
def observation_factory(env: 'AbstractEnv', config: dict) -> ObservationType:
    if config["type"] == "EnvWorld":
        return EnvObservation(env, **config)
    if config["type"] == "OccupyGrid":
        return SingleAgentObservation(env, **config)
