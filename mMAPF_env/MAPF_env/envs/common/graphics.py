...
from ...world.graphics import WorldGraphics, WorldSurface
...
class EnvViewer(object):
    ...
    def __init__(self, env: "AbstractEnv", config: Optional[dict] = None) -> None:
        ...
        pygame.init()
        pygame.display.set_caption("RLagents-env")
        ...
        WORLD_SHAPE = self.env.world.state.shape
        SCREEN_SHAPE = np.array(list(WORLD_SHAPE)) * self.SCALE_SIZE
        if not self.offscreen:
            # self.screen = pygame.display.set_mode(
            #     [self.config["screen_width"], self.config["screen_height"]]
            # )
            self.screen = pygame.display.set_mode(
                [SCREEN_SHAPE[0], SCREEN_SHAPE[1]]
            )
        if self.agent_display:
            self.extend_display()

        self.sim_surface = WorldSurface(SCREEN_SHAPE, 0, pygame.Surface(SCREEN_SHAPE))
    ...
    def display(self) -> None:
        ...
        WorldGraphics.display(self.env.world, self.sim_surface)
        for agent in self.env.agents:
            AgentGraphics.display(agent, self.sim_surface)
        ...
        # WorldGraphics.display_road_objects(
